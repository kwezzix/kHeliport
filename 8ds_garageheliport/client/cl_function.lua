...
function SpawnVehicle(vehicle, plate)
	...
	TriggerServerEvent('helico:breakVehicleSpawn', plate, false)
end
...
function ReturnVehicle()
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('helico:returnVehicle', function(valid)
			if valid then
...
function BreakReturnVehicle(vehicle, vehicleProps)
	...
	TriggerServerEvent('helico:breakVehicleSpawn', vehicleProps.plate, true)
			...
			if dist <= Config.MarkerDistance then
				...
				wait = 1
					...
					if IsControlJustPressed(1,51) then
						ESX.TriggerServerCallback('helico:vehiclelist', function(ownedCars)
		...
		local wait = 750
			...
			if dist <= Config.MarkerDistance then
				...
				wait = 0
					...
					if IsControlJustPressed(1,51) then
						ESX.TriggerServerCallback('helico:vehiclelistfourriere', function(ownedCars)
...
Citizen.CreateThread(function()
    if Config.Blip then
        for k, v in pairs(Config.Positions.Garage) do
            ...
            AddTextComponentSubstringPlayerName('Garage d\'hélicoptère')
		...
		for k, v in pairs(Config.Positions.Pound) do
            ...
            BeginTextCommandSetBlipName('STRING')
            AddTextComponentSubstringPlayerName('Fourrière d\'hélicoptère')
            EndTextCommandSetBlipName(blip)
        end
    end
end)