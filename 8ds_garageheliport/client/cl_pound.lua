...
-- Menu --
local open = false 
local MenuPound = RageUI.CreateMenu("Fourrière Hélicoptère", "INTERACTION")
    ...
    if open then 
        ...
        Citizen.CreateThread(function()
            while open do 
                ...
                RageUI.Separator("↓     ~b~Hélicoptère     ~s~↓")
                for i = 1, #Pound.poundlist, 1 do
                    ...
                    RageUI.Button(nomvehicletexte.." | "..plaque, nil, {RightLabel = "~y~→"}, true, {
                        onSelected = function() 
                            ESX.TriggerServerCallback('helico:achat', function(suffisantsous)
